...
ProfilerPass::ProfilerPass() :
		ModulePass(ID), DL(nullptr), TLIWrapper(nullptr), TLI(nullptr), TotalFuncs("TotalFuncs",
				...
				"TotalJoins",
				"Number of basic blocks with more than one predecessor"), TotalInsts(
				"TotalInsts", "Number of instructions"), TotalDirectCalls(
				"TotalDirectCalls", "Number of direct calls"), TotalIndirectCalls(
				"TotalIndirectCalls", "Number of indirect calls"), TotalAsmCalls(
				"TotalAsmCalls", "Number of assembly calls"), TotalExternalCalls(
				"TotalExternalCalls", "Number of external calls"), TotalUnkCalls(
				"TotalUnkCalls", "Number of unknown calls"), TotalLoops(
				"TotalLoops", "Number of loops"), TotalBoundedLoops(
				"TotalBoundedLoops", "Number of bounded loops")
				////////
				, SafeIntDiv("SafeIntDiv", "Number of safe integer div/rem"), SafeFPDiv(
				"SafeFPDiv", "Number of safe FP div/rem"), UnsafeIntDiv(
				"UnsafeIntDiv", "Number of definite unsafe integer div/rem"), UnsafeFPDiv(
				"UnsafeFPDiv", "Number of definite unsafe FP div/rem"), DivIntUnknown(
				"DivIntUnknown", "Number of unknown integer div/rem"), DivFPUnknown(
				"DivFPUnknown", "Number of unknown FP div/rem")
				/////////
				, TotalMemInst("TotalMemInst", "Number of memory instructions"), MemUnknown(
				"MemUnknown", "Statically unknown memory accesses"), SafeMemAccess(
				"SafeMemAccess", "Statically safe memory accesses"), TotalAllocations(
				"TotalAllocations", "Malloc-like allocations"), InBoundGEP(
				"InBoundGEP", "Inbound GetElementPtr"), MemCpy("MemCpy"), MemMove(
				"MemMove"), MemSet("MemSet")
		/////////
				, SafeLeftShift("SafeLeftShift", "Number of safe left shifts"), UnsafeLeftShift(
				"UnsafeLeftShift", "Number of definite unsafe left shifts"), UnknownLeftShift(
...
bool ProfilerPass::runOnModule(Module &M) {
	...
	if (ShowCallGraphInfo) {
		...
		for (auto it = scc_begin(&CG); !it.isAtEnd(); ++it) {
			...
			for (CallGraphNode *cgn : scc) {
				if (cgn->getFunction()
					...
					funcs.push_back(
							{ cgn->getFunction(), { cgn->getNumReferences(),
		...
		for (auto &p : funcs) {
			...
			CGcounter func (F->getName(), F->getBasicBlockList().size(), numInsts ,p.second.first, p.second.second);
			functions.push_back(func);
			errs() << F->getName() << ":" << " num of BasicBlocks="

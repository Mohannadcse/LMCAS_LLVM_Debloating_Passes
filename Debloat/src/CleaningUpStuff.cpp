...
void CleaningUpStuff::dfsutils(
    CallGraph &cg, Function *f,
      ...
      if (n->second) {
        ...
        for (int i = 0; i < cgn->size(); i++) {
          if (cgn->operator[](i)->getFunction() &&
              cgn->operator[](i)->getFunction()->isDeclaration()) {
            strLogger << "\tidx" << i << ": "
                      << cgn->operator[](i)->getFunction()->getName() << "\n";
            reachableFunctionsFromMain.insert(
                cgn->operator[](i)->getFunction());
            dfsutils(cg, cgn->operator[](i)->getFunction(),
                     reachableFunctionsFromMain);
          }
        }
      }
    }
...
void CleaningUpStuff::removeUnusedStuff(Module &module) {
  ...
  /*
   * The entry point is the mian function. Therefore, I need to identify all
   * called methods inside the main I store them in a set (i don't want repeated
   * nodes) then I can get the invoked functions from each function Here I don't
   * care about constructing paths from main to reachable functions
   */
  std::set<const Function *> reachableFunctionsFromMain;
  ...
  for (auto curF = module.getFunctionList().begin();
    ...
    if (reachableFunctionsFromMain.find(&curF->getFunction()) ==
        reachableFunctionsFromMain.end()) {
      Function &f = const_cast<Function &>(curF->getFunction());
      strLogger << "\tRemoving process of func: " << f.getName()
                << "-- Uses: " << curF->getNumUses() << "\n";
      for (User *usr : f.users()) {
        if (Instruction *inst = dyn_cast<Instruction>(usr)) {
          strLogger << "\t\tUsers: " << *inst
  ...
  for (auto curF = module.getFunctionList().begin();
    ...
    if (curF->getName() != "main") {
      //			strLogger << "FuncName: " << curF->getName() <<
      //" NumUses= " << curF->getNumUses() << "\n";
      if (curF->getNumUses() == 0)
  ...
  for (auto curF = module.getFunctionList().begin();
    ...
    if (std::find(funcToBeRemoved.begin(), funcToBeRemoved.end(), &*curF) !=
      ...
      // I created 2 Instruction vectors: aloc instrs and stor instr. Because I
      // need to remove store instrs before alloc instr otherwise, I'll receive
      // errors if the alloc was removed before its store insr
      for (auto I = inst_begin(*curF); I != inst_end(*curF); I++) {
        Instruction *i = &*I;
        if (auto ai = dyn_cast<AllocaInst>(i)) {
          //						strLogger << "Var: " << *ai << " , Used = "
          //<< ai->getNumUses() << "\n";
          for (auto u : ai->users()) {
            //							strLogger << "\tUses: " << *u
            //<<"\n";
  ...
  /// the following 4 for-loops remove the identified unused variables and
  /// functions

...
/*
 * this method checks if the gep is matching any of the pointer to struct
 * variables
 */
bool LocalVariables::processGepInstrPtrStruct(
  ...
  if (gep->getNumOperands() == 3) {
    ...
    auto opr0InstrLD =
        dyn_cast<LoadInst>(gep->getOperand(0)); // to handle pointer to struct
      ...
      if (auto pt = dyn_cast<StructType>(opr0Type->getElementType()))
        if (auto op2 = dyn_cast<ConstantInt>(gep->getOperand(2)))
          if (pt->getStructName() == get<0>(structInfo) &&
              ...
              returnIndex(instList, cast<Instruction>(
                                        opr0InstrLD->getPointerOperand())) ==
                  get<2>(structInfo))
            ret = true;
          else
            ret = false;
...
bool LocalVariables::processGepInstrStruct(
    llvm::GetElementPtrInst *gep, tuple<string, uint64_t, int> structInfo) {
  bool ret = false;
  if (gep->getNumOperands() == 3) {
    auto opr0Type = dyn_cast<PointerType>(gep->getOperand(0)->getType());
    ...
    if (opr0Type && opr0InstrAlloc) {
      if (auto pt = dyn_cast<StructType>(opr0Type->getElementType()))
        if (auto op2 = dyn_cast<ConstantInt>(gep->getOperand(2))) {
          if (pt->getStructName() == get<0>(structInfo) &&
              op2->getValue().getZExtValue() == get<1>(structInfo) &&
              returnIndex(instList, cast<Instruction>(opr0InstrAlloc)) ==
                  get<2>(structInfo))
            ret = true;
          else
            ret = false;
...
/*
 * cntxtFlg is used to recognize different contexts where this function is
 * called. This flag is also used for handling variables called by address when
 * their address are passed as parameters, so we don't need to check the neck
 * location we achieve this by setting the value of this flag to zero
 */
bool LocalVariables::processGepInstrNestedStruct(
  ...
  if (mainGEP->getNumOperands() == 3 && elemGEP->getNumOperands() == 3) {
    auto opr0TypeMain =
        dyn_cast<PointerType>(mainGEP->getOperand(0)->getType());
    auto opr0InstrAllocMain = dyn_cast<AllocaInst>(mainGEP->getOperand(0));
    auto opr0TypeElem =
        dyn_cast<PointerType>(elemGEP->getOperand(0)->getType());
    if (!cntxtFlg) {
      auto opr0InstrLoadMain = dyn_cast<LoadInst>(mainGEP->getOperand(0));
      opr0InstrAllocMain =
          dyn_cast<AllocaInst>(opr0InstrLoadMain->getOperand(0));
    }
    if (opr0TypeMain && opr0InstrAllocMain && opr0TypeElem) {
      ...
      if (ptMain && ptElem) {
        ...
        if (op2Main && op2Elem) {
          bool idxInst =
              returnIndex(instList, cast<Instruction>(opr0InstrAllocMain)) ==
              get<4>(structInfo);
          if (!cntxtFlg)
            idxInst = 1;

          if (ptMain->getStructName() == get<0>(structInfo) &&
              ptElem->getStructName() == get<1>(structInfo) &&
...
void LocalVariables::constantConversionStrctVars(
    Module &module, GetElementPtrInst *elemGEP, string funcName, uint64_t value,
  ...
  for (auto usr : elemGEP->users()) {
    if (auto si = dyn_cast<StoreInst>(usr)) {
      ...
      if (cntxtFlg == 1 || cntxtFlg == 2)
        idxInst =
            returnIndex(instList, si) < neckIndex(module, instList, funcName);
      //				we don't care about the idxInst in func
      // before the neck
      ...
      if (idxInst)
        if (auto ci = dyn_cast<ConstantInt>(si->getOperand(0))) {
          ...
          auto val = ConstantInt::get(si->getOperand(0)->getType(), value);
...
void LocalVariables::handleNestedStrct(
  ...
  for (auto elem : vars) {
    strLogger << "ELEM:: " << get<0>(elem.first) << " " << get<1>(elem.first)
              << " " << get<2>(elem.first) << " " << get<3>(elem.first) << " "
              << *instList[get<4>(elem.first)] << "\n";
    for (auto curF = module.getFunctionList().begin();
            ...
            for (auto usr : mainGEP->users()) {
              if (auto elemGEP = dyn_cast<GetElementPtrInst>(usr))
                if (processGepInstrNestedStruct(mainGEP, elemGEP, elem.first,
                                                1))
                  constantConversionStrctVars(module, elemGEP, funcName,
                                              elem.second, strLogger, 1);
            }
          }
        }

        // handling nested structs passed by address before the neck
        // it finds function calls wherein struct variables are passed by
        // address
        for (auto usr : instList[get<4>(elem.first)]->users())
          if (auto callInst = dyn_cast<CallInst>(usr)) {
            strLogger << "\tFound user: " << *callInst << "\n";
            if (returnIndex(instList, callInst) <
              ...
              for (auto arg = callInst->arg_begin(); arg != callInst->arg_end();
                ...
                if (instList[get<4>(elem.first)] == *arg) {
                  strLogger << "\t\ttr: "
                            << callInst->paramHasAttr(c, Attribute::ByVal)
                            << "\n";
                  if (!callInst->paramHasAttr(c, Attribute::ByVal)) {
                    strLogger
                        << "\t\t\tfound call by address to: " << *arg->get()
                        << "\n";
                    for (auto curI = inst_begin(*callInst->getCalledFunction());
                         curI != inst_end(*callInst->getCalledFunction());
                         curI++) {
                      if (auto *mainGEP = dyn_cast<GetElementPtrInst>(&*curI)) {
                        for (auto usr : mainGEP->users()) {
                          if (auto elemGEP = dyn_cast<GetElementPtrInst>(usr)) {
                            if (processGepInstrNestedStruct(mainGEP, elemGEP,
                                                            elem.first, 0))
                              constantConversionStrctVars(module, elemGEP,
                                                          funcName, elem.second,
                                                          strLogger, 0);
                          }
                        }
                      }
...
/*
 * The main logic to convert ptr to strct in a nestedstruct variables to
...
void LocalVariables::handlePtrToNestedStrct(
  ...
  // this logic for constant conversion for a struct variable that contains a
  // pointer to struct element because the pattern here is: GEP  main strct >
  // load instr > GEP elem strct > load instr (after neck) or store inst (before
  // neck)
  for (auto elem : vars) {
    strLogger << "ELEM:: " << get<0>(elem.first) << " " << get<1>(elem.first)
              << " " << get<2>(elem.first) << " " << get<3>(elem.first) << " "
...
/*
 * this function performs constant conversion after the neck for pointer to
 * nested struct after the neck iff the element's value hasnt been changed after
 * the neck by doing something similar to the function
...
/*
 * this function replaces pointer to strct but after the neck iff the struct
 * hasnt been modified
 */
  ...
  for (auto elem : gepInfo) {
    ...
    if (!get<0>(elem.second)) {
      for (auto ld : get<1>(elem.second)) {
        ...
        if (auto intType = dyn_cast<IntegerType>(
                ld->getPointerOperand()->getType()->getPointerElementType())) {
          auto val = llvm::ConstantInt::get(intType, get<2>(elem.second));
          strLogger << "\tReplace: " << *ld
                    << "  with Val: " << get<2>(elem.second) << "\n";
          ld->replaceAllUsesWith(val);
...
void LocalVariables::handlePtrToStructAfterNeck(
  ...
  for (auto srctGep : clocals) {
    ...
    gepInfo.push_back(make_pair(
        inst, make_tuple(isThereStoreInst, loadInstUseGep, srctGep.second)));
  }
...
void LocalVariables::handleStructLocalVars(
        ...
        for (auto curB = curF->begin(); curB != curF->end(); curB++) {
          for (auto curI = curB->begin(); curI != curB->end(); curI++) {
            if (auto gep = dyn_cast<GetElementPtrInst>(curI)) {
              if (returnIndex(instList, gep) <
                  neckIndex(module, instList, funcName)) {
                // check if the gep is matching one of the elements in
                // structLocals
                    ...
                    if (auto si = dyn_cast<StoreInst>(usr)) {
                      if (auto ci = dyn_cast<ConstantInt>(si->getOperand(0))) {
                        auto val = ConstantInt::get(
                            si->getOperand(0)->getType(), elem.second);
                        StoreInst *str = new StoreInst(val, si->getOperand(1));
                        strLogger << "\tSI uses GEP replace \n\t\tFROM: " << *si
                                  << "\tTO: " << *str << "\n";
                        ReplaceInstWithInst(si, str);
  ...
  logger << strLogger.str();
  //	handleLocalStructUsesAfterNeck(module, structLocals, modifiedInst,
  // funcName);
  inspectInitalizationPreNeck(module, structLocals, funcName);
}

/*
 * I need to decide which GEP operand that will be the allocate instr (I noticed
 * I need to check only operand 0) I need to do it now to handle only structs,
 * when GetElementPtrInst is used to compute the address of struct the number of
 * operands is 3. getelementptr t* %val, t1 idxInst1, t2 idxInst2
            ...
            if (auto gep = dyn_cast<GetElementPtrInst>(curI))
              if (returnIndex(instList, gep) <
                ...
                if (processGepInstrPtrStruct(gep, elem.first)) {
                  strLogger << "FOUND GEP with 3 args"
                            << "\n";
                  // this var counts the number of gep that have been converted
                  // to constant perform the constant conversion
                  for (auto i : gep->users()) {
                    strLogger << "GEP: " << *gep << "\n";
...
void LocalVariables::handleStructInOtherMethods(
    Function *fn, map<tuple<string, uint64_t, int>, uint64_t> &clocals,
  ...
  for (auto curI = inst_begin(fn); curI != inst_end(fn); curI++) {
    if (auto gep = dyn_cast<GetElementPtrInst>(&*curI)) {
      if (auto opr0Type = dyn_cast<PointerType>(gep->getOperand(0)->getType()))
        if (auto pt = dyn_cast<StructType>(opr0Type->getElementType()))
          if (auto op2 = dyn_cast<ConstantInt>(gep->getOperand(2))) {
            auto it = clocals.find(make_tuple(
                pt->getStructName(), op2->getValue().getZExtValue(), idxInst));
            if (it != clocals.end()) {
              for (auto i : gep->users()) {
                strLogger << "GEP: " << *gep << "\n";
                strLogger << "\tuser of GEP: " << *i << "\n";
...
/*
 * sometimes the struct variable is initalized in a different method than the
 * main method like the struct x in the main in `rm`: rm_option_init (&x) I need
 * to inspect these methods by checking: 1- all method calls before the main
 * method 2- check the arguments of each method if it contains the struct
 * variable,initalized
  ...
  for (auto inst : instList) {
    if (auto *ai = dyn_cast<AllocaInst>(inst)) {
      if (ai->getType()->isPointerTy()) {
        if (auto *st =
                dyn_cast<StructType>(ai->getType()->getPointerElementType())) {
...
void LocalVariables::handleLocalPrimitiveUsesAfterNeck(
  ...
  // TODO i might need to check the alloc isn't modified after the neck before
  // making CC I can check that by making sure no store instr users after the
  // neck
  ...
  for (auto var : updatedInstrToIdxInst) {
    ...
    for (auto usr : var.first->users()) {
      ...
      if (auto ld = dyn_cast<llvm::LoadInst>(usr)) {
        ...
        if (returnIndex(instList, cast<Instruction>(usr)) >
          ...
          // make sure the alloc instr has a name, some alloc instr don't have a
          // name, so I avoid making CC
          ...
          if (constVal != plocals.end())
            if (auto intType = dyn_cast<IntegerType>(
              ...
              strLogger << "\nLD replace.. \n";
              strLogger << "\tFOUND: " << *var.first << " :: " << var.second
                        << " :: " << *usr << "\n";
              ld->replaceAllUsesWith(val);
    ...
    //			if (ld->getNumUses() == 0) {
    //				ld->eraseFromParent();
    //				strLogger << "\t\tRemoving converted Load instr:
    //"
    //<< *ld
    //						<< "\n";
    //			}
...
void LocalVariables::handlePtrToPrimitiveLocalVariables(
    Module &module, map<uint64_t, pair<uint64_t, uint64_t>> &ptrToPrimtive,
    string funcName, Function *neckCaller) {
  string str;
  raw_string_ostream strLogger(str);
  ...
  // here I need to find the alloc instr based on actualIdxInst not the
  // ptrIdxInst
  ...
  for (auto curF = module.getFunctionList().begin(),
    ...
    if (fn == "main") {
      ...
      for (auto curB = curF->begin(); curB != curF->end(); curB++) {
        for (auto curI = curB->begin(); curI != curB->end(); curI++) {
          if (returnIndex(instList, &*curI) <
              neckIndex(module, instList, funcName)) {
            // handle load inst
            if (auto ld = dyn_cast<LoadInst>(curI)) {
              strLogger << "ptr oprd: " << *ld->getPointerOperand() << "---"
                        << *ld->getPointerOperand()->getType() << "\n";
              //							auto al
              //= dyn_cast<AllocaInst>(ld->getOperand(1));
              // auto inst =
              // instrToIdxInst.find(cast<AllocaInst>(si->getOperand(1)));
              // auto val = llvm::ConstantInt::get(intType,
              // inst->second.second);
              //							ReplaceInstWithValue(curB->getInstList(),
              // curI,
              // val); handle store inst
            } else if (auto si = dyn_cast<StoreInst>(curI)) {
              if (returnIndex(instList, si) <
                  neckIndex(module, instList, funcName)) {
                strLogger << "SI:  " << *si << "\n";
                // strLogger << "\toperands:: " << si->getNumOperands() << "
                // ---opr0: " << *si->getOperand(0) <<  " ---opr1: " <<
                // *si->getOperand(1) << "\n";

                if (isa<llvm::PointerType>(si->getOperand(1)->getType()) &&
                  ...
                  if (inst != instrToIdxInst.end()) {
                    ...
                    if (auto intType = dyn_cast<IntegerType>(
                      ...
                      auto val =
                          llvm::ConstantInt::get(intType, inst->second.second);
                      strLogger << "\nSI replace.. \n";
                      strLogger << "\tFOUND: " << *inst->first
                                << " :: " << inst->second.first
                                << " :: " << *curI << "\n";
                      StoreInst *str = new StoreInst(val, curI->getOperand(1));
                      ReplaceInstWithInst(curB->getInstList(), curI, str);
                      strLogger << "\tAfter conv: " << *curI << "\n";
                    }
                  }
                }
              }
...
void LocalVariables::handleStringVars(
  ...
  for (auto elem : strList) {
    ...
    Constant *ary = llvm::ConstantDataArray::getString(
        module.getContext(), elem.second.second, true);
    GlobalVariable *gv =
...
void LocalVariables::handlePrimitiveLocalVariables(
    Module &module, map<string, uint64_t> &plocals, string funcName,
    Function *neckCaller) {
  string str;
  raw_string_ostream strLogger(str);
  ...
  // I need to update plocals because I noticed some vars are included but their
  // declaration is after the neck. So I need to remove these stack vars
  map<string, uint64_t> updatedPlocals;

  strLogger << "*****\nFind list of matching instructions that have index in "
               "locals.\n";
  for (auto curF = module.getFunctionList().begin(),
            endF = module.getFunctionList().end();
       curF != endF; ++curF) {
    ...
    if (fn == "main") {
      // Get mapping between allocation instruction and its index in the locals
      // file this step is necessary to identify the corresponding value to each
      // local variable local variables are alloca instructions I noticed the
      // index of instructions isn't a reliable approach, because in the LLVM
      // pass we consider the entire bitcode, but KLEE interpreter doesn't
      // maintain the order of the instructions one hack I found, in KLEE there
      // is no 2 consecutive br instructions Therefore, when I reach a branch
      // instr: 1- check if there is a br instr before it 2- if so, reduce i by
      // one
      uint64_t i = 0;
      for (auto curB = curF->begin(); curB != curF->end(); curB++) {
        for (auto curI = curB->begin(); curI != curB->end(); ++curI) {
          ...
          if (auto al = dyn_cast<llvm::AllocaInst>(curI)) {
            if (id != plocals.end()) {
              if (!isa<StructType>(al->getType()->getElementType())) {
                stackVarToIdxInst.emplace(cast<llvm::AllocaInst>(curI), i);
              }
            }
            // obtain the names of local variables, if applicable
            if (!al->hasName()) {
              for (auto I = curB->begin(); I != curB->end(); I++) {
                if (DbgDeclareInst *dbg = dyn_cast<DbgDeclareInst>(&(*I))) {
                  if (const AllocaInst *dbgAI =
                          dyn_cast<AllocaInst>(dbg->getAddress()))
                    if (dbgAI == al) {
                      auto ff = returnIndex(instList, dbg) <
                                neckIndex(module, instList, funcName);
                      ...
                      if (DILocalVariable *varMD = dbg->getVariable()) {
                        strLogger << *al << "\n";
                        strLogger << "\tVarName: " << varMD->getName().str()
                                  << "\n";
                        instrToVarName.emplace(cast<llvm::AllocaInst>(curI),
                                               varMD->getName().str());
                      }
                    }
                }
  ...
  for (auto curF = module.getFunctionList().begin(),
    ...
    if (fn == "main") {
      ...
      for (auto curB = curF->begin(); curB != curF->end(); curB++) {
        for (auto curI = curB->begin(); curI != curB->end(); curI++) {
          // i need to add if store isnt and 1)its 2nd operand is alloc instr
          // and 2)1st operand is constvalue, the do the replacement
          if (auto ld = dyn_cast<LoadInst>(curI)) {
            //						strLogger << "lstSize: "
            //<< instList.size()
            //<< " LD idxInst: " << returnIndex(instList, ld) << "--" << *ld
            // <<"\n";
            if (returnIndex(instList, ld) <
                neckIndex(module, instList,
                          funcName)) // do the conversion till the neck
              if (auto opr =
                      dyn_cast<llvm::AllocaInst>(ld->getPointerOperand())) {
                ...
                if (inst != stackVarToIdxInst.end()) {
                  auto constVal = plocals.find(std::to_string(inst->second));
                  if (constVal != plocals.end()) {
                    if (auto intType = dyn_cast<IntegerType>(
                            opr->getType()->getElementType())) {
                      auto val =
                          llvm::ConstantInt::get(intType, constVal->second);
                      strLogger << "\nLD replace.. \n";
                      strLogger << "\tFOUND: " << *inst->first
                                << " :: " << inst->second << " :: " << *curI
                                << "\n";
                      ReplaceInstWithValue(curI->getParent()->getInstList(),
                                           curI, val);
                    }
                  }
                }
            ...
            if (returnIndex(instList, si) <
              ...
              //<< "SI:  " << *si << "\n";
              // strLogger << "\toperands:: " << si->getNumOperands() << "
              // ---opr0: " << *si->getOperand(0) <<  "
              //---opr1: " << *si->getOperand(1) << "\n";
                  ...
                  if (inst != stackVarToIdxInst.end()) {
                    ...
                    if (constVal != plocals.end()) {
                      if (auto intType = dyn_cast<IntegerType>(
                        ...
                        StoreInst *str =
                            new StoreInst(val, curI->getOperand(1));
                        ReplaceInstWithInst(curB->getInstList(), curI, str);
                        strLogger << "\tAfter conv: " << *curI << "\n";
                      }
                    }
...
void LocalVariables::testing(Module &module) {
  ...
  for (auto f = module.getFunctionList().begin();
    ...
    strLogger << "Func: " << f->getName() << "---#USERS: " << f->getNumUses()
              << "\n";
